...
public class UserService
{
    ...
    public async Task<List<EmployeeModel>> GetEmployeesForTRGDVAsync()
    {
        ...
        using (var conn = new OracleConnection(_connectionString))
        {
            ...
            string queryNVCB = @"SELECT MANLD, HOTEN, PHAI, NGSINH, DT, VAITRO, MADV, LUONG, PHUCAP 
                           FROM ADMINPDB.NHANVIEN_NVCB";

            using (var cmd = new OracleCommand(queryNVCB, conn))
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                while (await reader.ReadAsync())
                {
                    employees.Add(new EmployeeModel
                    {
                        ...
                        Salary = reader.IsDBNull(reader.GetOrdinal("LUONG")) ? 0 : reader.GetDecimal(reader.GetOrdinal("LUONG")),
                        Allowance = reader.IsDBNull(reader.GetOrdinal("PHUCAP")) ? 0 : reader.GetDecimal(reader.GetOrdinal("PHUCAP"))
                    });
                }
            }

            // 2. Lấy thông tin từ view NHANVIEN_TRGDV (không có lương và phụ cấp)
            string queryTRGDV = @"SELECT MANLD, HOTEN, PHAI, NGSINH, DT, VAITRO, MADV 
                            FROM ADMINPDB.NHANVIEN_TRGDV";

            using (var cmd = new OracleCommand(queryTRGDV, conn))
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                while (await reader.ReadAsync())
                {
                    employees.Add(new EmployeeModel
                    {
                        ...
                        Role = reader.GetString(reader.GetOrdinal("VAITRO")),
                        Department = reader.GetString(reader.GetOrdinal("MADV")),
                        Salary = 0,  // Không có thông tin lương từ view này
                        Allowance = 0 // Không có thông tin phụ cấp từ view này
                    });
                }
            }
        }
