    ...
    public class NotificationViewModel : ViewModelBase
    {
        ...
        private void LoadNotifications()
        {
            try
            {
                ...
                {
                    conn.Open();
                    var cmd = new OracleCommand("SELECT MATB, TIEUDE, NOIDUNG, NGAYTB,LABEL_TO_CHAR(OLS_LABEL) as OLS_LABEL FROM OLS_ADMIN.THONGBAO ORDER BY NGAYTB DESC", conn);
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            Notifications.Add(new Notification
                            {
                                ...
                                OLS_LABEL = reader["OLS_LABEL"].ToString(),
